...
def outFC_df(newdf, metas, contrast,  eps):
    ...
    fctab = pd.DataFrame(data={"mets":mets, "FC_geommu":FC_geommu,
                        "log2FC" : np.log2(FC_geommu) })
...
def outStat_df(newdf, metas, contrast,  whichtest):
    ...
    prediffr = pd.DataFrame(data={"mets":mets, "stat":stare,
                        "pval":pval })    
    return prediffr
...
def rundiffer(datadi, tablePicked, namesuffix, metadata,
         ordercontrast,
          contrast,
          whichtest,
         technical_toexclude, co,  outdiffdir, choice):
    """
    ...
    eps = 1
    if choice == "TOTAL":
        abun = pd.read_csv(datadi+ tablePicked + "_"+namesuffix + \
    ...
    newdf_tot, metas_tot = prepare4contrast(abun, metadahere,
                                            ordercontrast, contrast)
    if reduce:
        df = newdf_tot.copy()
        mets = df.index
    ...
    prediffresult = outStat_df(newdf_tot_red,  metas_tot, contrast,
                               whichtest)

    cpdiff = prediffresult.copy()
    cpdiff["pval"] = cpdiff[["pval"]].fillna(1)  # inspired from R documentation in p.adjust
    (sgs, corrP, _, _) = ssm.multipletests(cpdiff["pval"],
    ...
    DIFFRESULT = pd.concat([prediffresult.set_index("mets"),
                            FCresult.set_index("mets") ],
