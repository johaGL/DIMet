...
def save_pca_plots(title, pc_df, var_explained_df, col1, col2, pointlabels,
    ...
    try:
        ...
        # print("ellipses args not set == > ", indexerr, ". Not drawn by default")
        pass
    ...
    if pointlabels != "":
        ...
        for i, row in pc_df.iterrows():
            ax.text(pc_df.at[i, 'PC1'] + 0.2, pc_df.at[i, 'PC2'] , pc_df.at[i, pointlabels],
                    size='x-small')
    # end if

    plt.xlabel(f"{var_explained_df.iloc[0, :]['PC']} {round(var_explained_df.iloc[0, :]['var'], 2)} %")
    plt.ylabel(f'{var_explained_df.iloc[1, :]["PC"]} {round(var_explained_df.iloc[1, :]["var"], 2)} %')
    plt.title(title)
    ...
    plt.savefig(f'{out_plot_dir}pca_{title.replace(" ", "-")}_label{yesnolabel}.pdf',
                format="pdf")
