...
def stackallabundace(abu_sel, metada_sel):
    ...
    dafull = pd.DataFrame(columns=["timepoint", "condition",  "metabolite", 
                                     "abundance"])
    for z in range(len(metabolites)):    
      ...
      dafull = pd.concat([dafull,subdf], ignore_index=True)  # TODO : replace df4plot.append by pd.concat([]) elsewhere
    return dafull
...
def mean_sd_D(newdf, ametadasel):
    ...
    for cd in condis:
        for t in times:
            sasas = ametadasel.loc[(ametadasel['condition'] == cd) & \
                                   (ametadasel['timepoint'] == t)]
...
def tmpstack(oD):
    oudf = pd.DataFrame(data={"metabolite": [],
                              "condition":[],
                              "timepoint": [],
                              "mean":[],
                              "sd" : []})
    ...
    for k in list(oD.keys())[1:]:
        ...
        tmp = pd.DataFrame(data={"metabolite": mets,
...
def printtestpluslegend(zoo, selectedmets, CO, SMX , plotwidth, odirbars):
    ...
    for il in range(len(ili)):
        ...
        for p in axs[il].patches : 
            ...
            wp = p.get_width()
            yp = p.get_height()
            axs[il].errorbar( xp + (wp/2) , 
                             yp, 
                       yerr = icialter["sd"].tolist()[isd],
                         fmt = 'none', capsize = 10, 
                         ecolor = 'black', alpha=0.9,
                         zorder = 1) # zorder -1 behind color bar
            isd += 1
        # end for p
    #end for il
...
def printabundbarswithdots(piled_sel,  selectedmets, CO, SMX , plotwidth, odirbars):
    ...
    for il in range(len(li_)):
        ...
        sns.barplot(ax = axs[il],
                    ...
                    alpha = 1,
                    edgecolor='black',
                    errcolor = 'black', errwidth = 1.7,
                    ci = "sd", capsize=.2)
        sns.stripplot( ax=axs[il],
                      data=herep,
                      x="condition",
                      y="abundance",
                      hue="timepoint",
    ...
    fig.suptitle(f"{CO} {SMX}".upper())
    plt.subplots_adjust( top=0.76, bottom=0.2,wspace=0.6, 
                        hspace=0.5)
